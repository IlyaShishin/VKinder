...
from package_data.vk_user import VkUser
from package_data.message_sender import TOKEN, vk, write_msg, send_photo
from package_data.connect_database import session, Candidates
                                                                ...
                                                                if event.type == VkEventType.MESSAGE_NEW and event.to_me:
                                                                    ...
                                                                    if int(request) < 0:
                                                                        ...
                                                                        users_search = vk_client.users_search(
                                                                            ...
                                                                            age_to)
                                                                        # Цикл выбирает id кандидата из списка,
                                                                        # Отсылает его фото пользователю
                                                                        # И удаляет id из списка
                                                                        sess = session()
                                                                        sess.query(
                                                                            Candidates).delete()
                                                                        candidates_list = []
                                                                        while True:
                                                                            for candidate_id in users_search:
                                                                                ...
                                                                                candidates_data = Candidates(name=f'vk.com/id{candidate_id}')
                                                                                sess.add(candidates_data)
                                                                                sess.commit()
                                                                                add_result = sess.query(Candidates).all()
                                                                                    ...
                                                                                    if event.type == VkEventType.MESSAGE_NEW and event.to_me:
                                                                                        ...
                                                                                        if request == 'да':
                                                                                            ...
                                                                                            write_msg(
                                                                                                event.user_id,
                                                                                                f'Поиск закончен. '
                                                                                                f'Вот список всех найденных пользователей:{candidates_list}'
                                                                                            )
                                                                            break
